...
#define WLAN_DEVICE_NAME "wlan0" //“wlan0”, "eth0"
#define PCM_BUFFER_SIZE 4194304 //4*1024*1024
#define PCM_PLAY_BUFFER 1024*64
...
//	SVP_INFO("Head: %s", jsonStr.c_str());
	...
	if (songid == pQQData->songid)
	{
		...
		CUtils::JsonToValue(&item["Length"], packLen);
		CUtils::JsonToValue(&item["TotalLength"], pQQData->totalLen);
		SVP_INFO("Head: packageindex %d, len %d, totalLen %d",
				pQQData->packindex, packLen, pQQData->totalLen);
		pQQData->ChangePack(pQQData->packindex,packLen);
		//拷贝剩余数据
...
//	std::lock_guard<std::recursive_mutex> lock(m_mtxPcmData);
	...
	if (!m_stCurrMediaInfo.Valid() || m_PCMData.totalLen <= m_PCMData.copyedLen \
			|| m_PCMData.localsize > PCM_BUFFER_SIZE )
		return;
...
{
	CQQData::pcmblock_pt pt_pcmblock = new CQQData::pcmblock_t();
	int len,pack_playpos,pack_playsize,sizepersecond;
	int playpos,byte_playpos;
	static std::string songid;
	stQQMediaInfo mediaInfo;
	bool ret;
	while(true)
	{
		m_PCMData.resetblock(pt_pcmblock);
		ret = m_PCMData.GetData(pt_pcmblock);
		len = pt_pcmblock->packcopyedLen;
		if (ret &&len > 0)
		{
			SVP_INFO("gyh====GetData : %d | local size:%d ",len,m_PCMData.localsize);
			if( mediaInfo != m_stCurrMediaInfo)
			{
			...
			while( unplay > 0 )
			{
				...
				unplay > PCM_PLAY_BUFFER ? pack_playsize = PCM_PLAY_BUFFER : pack_playsize = unplay;
				ret = SVPSingleton<SVPPcmPlayer>::getInstance()->PlayPcmData(pt_pcmblock->buffer.data() + pack_playpos,pack_playsize );
				pack_playpos += pack_playsize;
				unplay = len - pack_playpos;
				byte_playpos += pack_playpos;
				playpos = byte_playpos/sizepersecond;
				if (playpos > m_stCurrMediaInfo.id3.duration || byte_playpos == m_PCMData.totalLen)
					playpos = m_stCurrMediaInfo.id3.duration;
				if( byte_playpos % sizepersecond  == 0 )
				{
					m_stCurrMediaInfo.playpos = playpos;
